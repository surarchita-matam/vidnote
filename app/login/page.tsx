...
export default function LoginPage() {
  return (
    <div className="flex flex-1 items-center justify-center">
      <Suspense fallback={<LoginCard onSignIn={() => void 0} />}>
        <LoginInner />
      </Suspense>
    </div>
  );
...
function LoginInner() {
  ...
  return <LoginCard onSignIn={() => signIn("google", { callbackUrl })} />;
}
  ...
  return (
    <div className="w-full max-w-sm space-y-8 rounded-xl border bg-card p-8 shadow-sm">
      <div className="flex flex-col items-center text-center">
        ...
        <h1 className="mt-4 text-2xl font-semibold">VidNote</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Sign in to upload, annotate, and summarize videos.
        </p>
      </div>
